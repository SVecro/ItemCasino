...
public class UpgraderMenu extends AbstractCasinoMenu {
     ...
     */
    public UpgraderMenu(int containerId, Inventory playerInventory, RegistryFriendlyByteBuf buffer) {
    ...
    public UpgraderMenu(int containerId, Inventory playerInventory, CasinoSession session, Player viewer) {
        this(containerId, playerInventory, session.wagerContainer(),
                serverData(session, viewer), session);
    }

    private UpgraderMenu(int containerId, Inventory playerInventory,
                  net.minecraft.world.Container container,
                  net.minecraft.world.inventory.ContainerData data,
                  @Nullable CasinoSession session) {
        super(CasinoMenus.UPGRADER.get(), containerId, playerInventory, container, data, session, CasinoLayout.WAGER_X, CasinoLayout.WAGER_Y);
    }
}
